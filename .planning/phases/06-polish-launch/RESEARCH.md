# Research: Phase 6 – Polish, Analytics & Launch

**Roadmap**: Phase 6: Polish, Analytics & Launch  
...
- **Core Web Vitals**: Not instrumented; no `reportWebVitals` or LCP/FID/CLS reporting. Phase 6 should add minimal CWV reporting (e.g. in `main.tsx` or App) and fix any regressions found.
...
- **Offline/errors**: Phase 4 PWA will cache shell; Phase 6 should add user-facing messaging when offline or when sync/auth fails (e.g. banner or inline message), and a minimal runbook for deploy/support (env vars, Supabase, how to clear cache).

### 5. Dependencies

- **Phase 4**: Should be complete or near-complete (Supabase, Teacher Dashboard, Lick Feed, PWA).
- **Phase 5**: Director Engine (FSRS + context injection). Polish can proceed in parallel or after; analytics/onboarding may optionally use Director-driven session data.
