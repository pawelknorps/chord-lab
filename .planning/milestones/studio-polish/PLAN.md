# Studio Polish – Execution Plan

---
waves:
  - "Wave 1: Parallel Compression Bus (REQ-STUDIO-01, 02, 03)"
  - "Wave 2: Air Band and Master LUFS (REQ-STUDIO-04, 05)"
  - "Wave 3: Note Waterfall 60fps (REQ-STUDIO-06)"
dependencies:
  - "Existing globalAudio parallel bus and drums Air (Phase 22)"
  - "JazzKiller band and NoteWaterfall component"
...
When a user puts on headphones, the app must **sound like a mastered record, not a MIDI file** (what justifies $29/mo). Deliver: parallel compression 70/30 with 8:1 fast-attack wet bus, Drum Bus +2 dB @ 12 kHz, Master ~-14 LUFS, and Note Waterfall at 60fps decoupled from audio ticks.
...
<task>
  ...
  <description>
    In globalAudio.ts, set drumsAirBand (BiquadFilterNode / Tone.Filter high-shelf at 12 kHz) gain to 2 dB (REQ-STUDIO-04). Currently gain is 3; change to 2 so "Air" band matches Studio Polish spec.
  </description>
...
<task>
  ...
  <description>
    Implement master output leveling so that typical playback (trio + user) measures approximately -14 LUFS (within ~1 dB). Options: (1) Add a gain node before masterLimiter and set gain (or limiter threshold) so that when measured with an external LUFS meter the output is ~-14 LUFS; (2) optionally implement a short-term LUFS meter in a Worklet and adjust gain periodically. Document the target and how to verify (e.g. "Measure with browser extension or DAW; target -14 LUFS"). Ensure no clipping (limiter ceiling remains).
  </description>
...
<task>
  ...
  <description>
    In NoteWaterfall.tsx, stop using Tone.Transport.seconds for the animation time base. When a note is pushed via onNoteEvent, store startTime using wall-clock time (e.g. performance.now() / 1000 or a ref that is updated each rAF). In the animate() loop, use the same wall-clock time for "now" so that age = now - note.startTime is driven by display refresh, not Transport. Keep note list and layout logic unchanged; only the time base for scroll position becomes independent of Transport. This prevents stutter when the audio thread or Transport is under load.
  </description>
...
<task>
  ...
  <description>
    Visually verify waterfall under normal load and (if possible) under stress; confirm 60 updates/s or smooth animation. Update STATE.md Phase 3 checklist.
  </description>
...
**Phase complete when**: All six requirements are met, STATE.md is updated, and A/B listening confirms "mastered record" feel (punch, air, consistent level, smooth waterfall).
