...
- **Tasks**:
  - [ ] **REQ-HIFI-01**: Implement parallel bus: trio sum → dry Gain (e.g. 0.4) and wet path; dry + wet → masterBus (or current master chain).
  - [ ] **REQ-HIFI-02**: Make dry/wet gains configurable (constants or signals); verify "Parallel NY" balance (attack + body).
- **Deliverables**: globalAudio.ts refactor; optional Mixer toggle to bypass parallel (fallback to current path) for safe rollout.
...
- **Tasks**:
  - [ ] **REQ-HIFI-03**: Implement soft-knee compression in an Audio Worklet (custom or Essentia.js); or document fallback and implement in follow-up.
  - [ ] **REQ-HIFI-04**: Set default ratio ~4, knee ~30, attack ~5 ms, release ~150 ms (or adaptive).
  - [ ] **REQ-HIFI-05**: Ensure piano, bass, drums (and reverb sends if desired) feed the parallel mixer correctly.
- **Deliverables**: `wasm-compressor-processor` (or equivalent) registered; globalAudio wet path uses it; parameter struct in processorOptions.
...
- **Tasks**:
  - [ ] **REQ-HIFI-06**: Add RMS measurement (pre-compression input and post-sum output); 100–300 ms window, periodic update.
  - [ ] **REQ-HIFI-07**: Apply automatic makeup gain node (Gain = InputRMS/OutputRMS, bounds 0.25–4.0).
  - [ ] **REQ-HIFI-08**: Add "Mastering" / "Pro Mix" toggle in Mixer (or globalAudio); when on, parallel path + makeup are active; when off, dry-only or previous chain.
...
- **Tasks**:
  - [ ] **REQ-HIFI-09**: Add high-shelf EQ (+3 dB, 12 kHz) on drums path; Tone.EQ3 or BiquadFilter; optional bypass or tied to Pro Mix.
  - [ ] **REQ-HIFI-10**: WebGPU EQ deferred unless already in use.
- **Deliverables**: Drums Air band in globalAudio; optional Mixer control; listening test.
