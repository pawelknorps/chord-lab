...
drop policy if exists "Users can insert own song_progress" on public.song_progress;
create policy "Users can insert own song_progress"
  on public.song_progress for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own song_progress" on public.song_progress;
create policy "Users can update own song_progress"
  on public.song_progress for update
  using (auth.uid() = user_id);

drop policy if exists "Users can delete own song_progress" on public.song_progress;
...
drop trigger if exists song_progress_updated_at on public.song_progress;
...
drop policy if exists "Teachers can manage students in own classrooms" on public.classroom_students;
...
drop policy if exists "Students can read own classroom memberships" on public.classroom_students;
...
drop policy if exists "Users can CRUD own licks" on public.licks;
create policy "Users can CRUD own licks"
  on public.licks for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

drop policy if exists "Anyone can read public licks" on public.licks;
