...
import { getModalInterchangeChords, getFunctionalNoteName } from '../../../../utils/theoryEngine';
import { getScaleChords, getChordNotes } from '../../../../core/theory';
import { GitBranch, Keyboard, Info, Play } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export function ModalInterchangeLevel() {
    ...
    const borrowedOptions = useMemo(() => getModalInterchangeChords(currentKey), [currentKey]);

    const playRound = useCallback(() => {
        setGameState('LISTENING');
        setFeedback('Listen to the Major Context...');
        ...
        setTimeout(() => {
            // Pick a borrowed chord
            let option = borrowedOptions[Math.floor(Math.random() * borrowedOptions.length)];
            let attempts = 0;
            while (history.includes(option.label) && attempts < 5 && borrowedOptions.length > 2) {
                option = borrowedOptions[Math.floor(Math.random() * borrowedOptions.length)];
                attempts++;
            }

            // Get MIDI notes for the borrowed chord
            const midiNotes = getChordNotes(option.root, option.quality, 4, 'Root Position');
            // Use label as context for spelling (e.g. 'iv')
            ...
            setFeedback(`Identify the Borrowed Chord...`);
            fetAudio.playChord(noteStrings, '1n');
            setGameState('GUESSING');
        }, timeOffset + 1000);
    }, [currentKey, borrowedOptions, history]);
    ...
    const handleCorrect = useCallback((question: any) => {
        ...
        addScore(Math.floor(180 * multiplier) + streak * 15);
        setGameState('RESOLVED');

        // Play resolution back to Tonic I
        const scaleChords = getScaleChords(currentKey, 'Major', 3);
        const tonicNotes = scaleChords[0].midiNotes.map(m => getFunctionalNoteName(m, currentKey));

        setTimeout(() => {
            fetAudio.playChord(tonicNotes, '2n');
        }, 500);
    ...
    const handleIncorrect = useCallback((question: any) => {
        setFeedback('MODAL MISALIGNMENT');
        addScore(0);
    ...
    return (
        <div className="flex flex-col items-center gap-12 w-full max-w-4xl fade-in relative z-10">
            <div className="text-center space-y-4">
                ...
                >
                    Detect harmonic color borrowed from parallel minor and related modes.
                </motion.p>
            </div>
                    ...
                    <div className="h-32 flex flex-col items-center justify-center relative z-10">
                        {gameState === 'IDLE' ? (
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                onClick={playRound}
                                className="px-14 py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black rounded-full text-xl shadow-[0_20px_40px_rgba(37,99,235,0.2)] transition-all border border-blue-400/20 uppercase tracking-widest italic"
                            >
                                START MODAL DRILL
                            </motion.button>
                        ) : (
                            ...
                            >
                                {borrowedOptions.map((opt, idx) => (
                                    <motion.button
                                        ...
                                        className="group relative p-6 bg-white/[0.03] hover:bg-blue-500/10 border border-white/5 hover:border-blue-500/50 rounded-2xl transition-all duration-300 shadow-lg text-center"
                                    >
                                        <div className="text-2xl font-black text-white group-hover:text-blue-400 italic tracking-tight">{opt.label}</div>
            ...
            >
                <div className="flex items-center justify-center gap-3 mb-3">
                    ...
                    <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500">Harmonic Briefing</h4>
                    <div className="h-[1px] w-8 bg-white/10" />
                </div>
                <p className="text-sm text-white/40 leading-relaxed font-medium italic">
                    Recognize shared tones between parallel scales. iv (Subdominant Minor) is the most frequent anchor.
                </p>
                <div className="mt-4 flex items-center justify-center gap-2 text-[10px] font-black uppercase text-white/20 tracking-widest">
                    ...
                    Focus: Parallel Minor Borrowings
                </div>
            </motion.div>
