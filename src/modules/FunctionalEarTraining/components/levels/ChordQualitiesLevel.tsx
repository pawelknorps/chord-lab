...
import { PianoKeyboard } from '../../../ChordLab/components/PianoKeyboard';
...
export function ChordQualitiesLevel() {
    ...
    useEffect(() => {
        // Enforce mode based on difficulty
    ...
    }, [quizMode, targetAnswer, difficulty, score.correct, addExperience, updateStreak]);

    const getCurrentDrillData = () => {
        switch (currentMode) {
            case 'Triads': return TRIADS;
            case 'Sevenths': return SEVENTHS;
            case 'Extensions': default: return EXTENSIONS;
        }
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
                        ...
                        <div className="flex gap-2 mt-1">
                            {(['Novice', 'Advanced', 'Pro'] as Difficulty[]).map(d => (
                                <button
                                    ...
                                    className={`px-2 py-0.5 rounded text-[10px] uppercase font-black transition-all ${difficulty === d ? 'bg-purple-500 text-white shadow-lg shadow-purple-500/20' : 'bg-white/5 text-white/40 hover:text-white'}`}
                                >
                                    {d}
                                </button>
                            ))}
                ...
                <div className="flex gap-2 bg-black/40 p-1 rounded-xl">
                    {(['Extensions', 'Triads', 'Sevenths'] as DrillMode[])
                        .filter(m => {
                            if (difficulty === 'Novice') return m === 'Triads';
                            if (difficulty === 'Advanced') return m !== 'Extensions';
                            return true;
                        })
                        .map(mode => (
                            <button
                                key={mode}
                                onClick={() => { setCurrentMode(mode); setQuizMode(false); setFeedback(null); }}
                                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${currentMode === mode
                                    ? 'bg-purple-600 text-white shadow-lg'
                                    : 'bg-transparent text-white/40 hover:text-white'}`}
                            >
                                {t(`earTraining.modes.${mode}`)}
                            </button>
                        ))}
                </div>
                ...
                <div className="flex items-center gap-6">
                    ...
                    <button
                        ...
                        className={`px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest transition-all ${quizMode ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20' : 'bg-white/10 text-white hover:bg-white/20'}`}
                    >
                        {quizMode ? 'Exit Lab' : 'Start Drill'}
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                    <div className="p-6 bg-white/5 rounded-2xl border border-white/5 space-y-4">
                        <h3 className="text-white text-sm font-bold flex items-center justify-between">
                            <span className="flex items-center gap-2">
                                <Target size={16} className="text-purple-400" />
                                {quizMode ? 'Identify Target' : 'Explore Relationships'}
                            </span>
                            {difficulty === 'Pro' && quizMode && (
                                <span className="text-[10px] bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full border border-purple-500/30">KEYBOARD INPUT REQUIRED</span>
                            )}
                        </h3>
                        ...
                        {difficulty === 'Pro' && quizMode ? (
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                {drillData.map((item: any) => (
                                    <button
                                        key={item.name}
                                        onClick={() => quizMode ? handleAnswer(item.name) : playDrillItem(item)}
                                        className={`py-4 rounded-xl font-mono text-sm transition-all border ${quizMode
                                            ? 'bg-black/20 hover:bg-purple-500/20 text-white border-white/5 hover:border-purple-500/50'
                                            : 'bg-black/20 hover:bg-white/10 text-cyan-400 border-white/5 hover:border-white/20'
                                            }`}
                                    >
                                        <div className="font-black">{item.name}</div>
                                        {!quizMode && <div className="text-[8px] text-white/30 mt-1 uppercase tracking-tighter">{item.flavor}</div>}
                                    </button>
                                ))}
                            </div>
                        )}
                        ...
                        {quizMode && (
                            <button
                                onClick={() => { setInputBuffer([]); nextQuestion(); }}
                                className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 text-[10px] uppercase font-black text-white/40 rounded-xl transition-all"
                            >
                                <RotateCw size={14} />
                                Replay Audio
                            </button>
                ...
                <div className="relative min-h-[250px] bg-black/60 rounded-3xl border border-white/5 flex flex-col items-center justify-center overflow-hidden p-8">
                    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
                        <div className="w-64 h-64 rounded-full bg-purple-500/5 blur-3xl animate-pulse" />
                        <div className="w-48 h-48 rounded-full bg-cyan-500/5 blur-3xl animate-pulse delay-700" />
                    </div>

                    {feedback ? (
                        <div className="text-center z-10 space-y-4 animate-reveal">
                            <div className={`text-4xl font-black ${feedback.includes('Correct') ? 'text-emerald-400' : 'text-red-400'}`}>
                                {feedback === 'Correct! 🎉' ? 'EXCELLENT' : 'NOT QUITE'}
                            </div>
                            <div className="text-white/40 text-sm font-bold uppercase tracking-[0.2em]">
                                {feedback.includes('Try again') ? `Expected: ${targetAnswer}` : (feedback.split(' - ')[1] || 'Dynamic Hearing Detected')}
                            </div>
                        </div>
                    ) : (
                        <div className="text-center z-10 space-y-4">
                            ...
                            <div className="text-white/20 text-xs font-black uppercase tracking-[0.3em]">
                                {quizMode ? 'Listening for Target...' : 'Select a quality to analyze'}
                            </div>
                        </div>
                    )}
                </div>
