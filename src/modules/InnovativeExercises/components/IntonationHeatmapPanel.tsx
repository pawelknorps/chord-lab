...
import { IntonationHeatmapV2 } from './visualizations/IntonationHeatmapV2';
...
export function IntonationHeatmapPanel({ initialParams }: IntonationHeatmapPanelProps) {
  const root = initialParams?.root ?? 'C';
  const scaleName = initialParams?.scale ?? 'major';
  const {
    scaleNotes,
    results,
    pitchHistory,
    ...
    stopDrone,
    reset,
  } = useIntonationHeatmap(root, scaleName);
  ...
  const handleStartDrone = async () => {
    ...
    if (!synthRef.current) {
      synthRef.current = new Tone.Synth({
        ...
        envelope: { attack: 0.1, sustain: 0.8, release: 1 },
        volume: -6,
      }).toDestination();
    }
    if (synthRef.current) {
  ...
  return (
    <div className={`${innovativePanelContainerClass} flex flex-col gap-6`}>
      ...
      <div>
        <p className={innovativeSectionLabelClass}>Pitch Contour</p>
        <IntonationHeatmapV2 pitchHistory={pitchHistory} width={400} height={100} />
      </div>
