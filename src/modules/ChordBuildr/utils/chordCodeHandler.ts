...
export const getChordDisplay = (chord: SelectedChord): string => {
  return `${chord.noteLetter}${chord.type}${isSlashChord(chord) ? `/${chord.slashNote}` : ""
    }`
}
...
export function buildProgFromCode(state: AppState, code: string): AppState {
  ...
  try {
    ...
    const searchParams = new URLSearchParams(code.startsWith("?") ? code : `?${code}`)
...
export function getStateParamsCode(state: AppState): string {
  ...
  const synthCode = getSynthCode(state)
  if (synthCode) params.set('s', synthCode);

  const progCode = getProgressionCode(state)
  if (progCode) params.set('p', progCode);
