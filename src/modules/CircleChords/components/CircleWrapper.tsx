...
export default function CircleWrapper({
    ...
    return (
        <div className="circle-container">
            <h4 className="my-2 text-sm text-gray-400 text-center">{circleTitle}</h4>
            <div className={'justify-center flex'}>
                ...
                >
                    <circle cx={cx} cy={cy} r={outer + 10} fill="transparent" stroke="#dee2e6" strokeWidth={2} strokeOpacity={0.1} />
                    {angles.map((a, i) => {
                        ...
                        const majFill = isMajCurrent ? '#06b6d4' : isMajIn ? '#ffffff30' : '#ffffff05'
                        const minFill = isMinCurrent ? '#06b6d4' : isMinIn ? '#ffffff30' : '#ffffff05'
                        ...
                        return (
                            <g key={i}>
                                <path d={majorPath} fill={majFill} stroke="#000000" strokeWidth={1} strokeOpacity={0.2}
                                    style={{ cursor: isMajIn ? 'pointer' : 'default' }}
                                    onClick={() => isMajIn && onSectorClick(true, i)} />
                                <path d={minorPath} fill={minFill} stroke="#000000" strokeWidth={1} strokeOpacity={0.2}
                                ...
                                <text x={majorTextPos[0]} y={majorTextPos[1]} textAnchor="middle"
                                    ...
                                    fill={isMajCurrent ? 'white' : isMajIn ? '#e5e5e5' : '#525252'}
                                    style={{ cursor: isMajIn ? 'pointer' : 'default' }}
                                ...
                                <text x={minorTextPos[0]} y={minorTextPos[1]} textAnchor="middle"
                                    ...
                                    fill={isMinCurrent ? 'white' : isMinIn ? '#a3a3a3' : '#404040'}
                                    style={{ cursor: isMinIn ? 'pointer' : 'default' }}
                    ...
                    <text x={cx} y={cy - 5} textAnchor="middle" dominantBaseline="middle"
                        fontFamily="Arial, sans-serif" fontSize={14} fontWeight="bold" fill="#ffffff">{keySig}</text>
                    <text x={cx} y={cy + 8} textAnchor="middle" dominantBaseline="middle"
                        fontFamily="Arial, sans-serif" fontSize={12}
                        fill="#a3a3a3">Chord Progression</text>
                </svg>
            </div>
        </div>
    )
